...
local function queue_next_dialogue_options()
	...
	if world:has(node, c.DialogueChoiceContainer) then
		...
		local choice_id = configs.dialogue_choice :: t.Entity
		node = choice_id
...
local function display_response_to_ui()
	...
	warn(`DISPLAYED THE RESPONSE MESSAGE TO UI!!!`)
end
...
local function display_choices_to_ui()
	...
	for_each_target(current_node, c.Next, function(id: jecs.Entity<unknown>)
